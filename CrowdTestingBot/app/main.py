...
async def start(client, message):
    ...
    try:
        ...
        query = f'''
            INSERT INTO
                candidates (id, username, name_from_telegram)
            VALUES 
...
async def about_cmd(client, message):
    ...
    await app.send_message(
        chat_id=user_id,
        text='''
📌 Мы погрузимся в мир тестирования платежных мобильных приложений для Android-смартфонов с NFC. Можно совершенно "с нуля" и без специфических знаний о тестировании присоединиться к команде.
...
Проект предлагает вам свободное исследование и нахождение дефектов - тут все ограничено только вашей фантазией! Найдите как можно больше ошибок и получите за это отдельное вознаграждение ✨
По секрету🤫: в прошлом этапе проекта было зафиксировано более 180 дефектов!
...
Если ошибка возникает в процессе оплаты, то такие ошибки оцениваются в два раза дороже. 
...
async def done(client, callback_query):
    ...
    await app.send_document(
        ...
        document=Path(
            dir_main,
            dir_data,
            'NDA (Соглашение о неразглашении).docx',
        ),
        caption='''
