import logging
import os
import threading
import time
from datetime import datetime, timedelta, timezone
from pathlib import Path
from warnings import filterwarnings
import pandas

import colorama
import psycopg
from telegram import (
    ForceReply,
    InlineKeyboardButton,
    InlineKeyboardMarkup,
    ReplyKeyboardMarkup,
    ReplyKeyboardRemove,
    Update,
)
from telegram.constants import ParseMode
from telegram.ext import (
    Application,
    CallbackQueryHandler,
    CommandHandler,
    ContextTypes,
    ConversationHandler,
    InvalidCallbackData,
    MessageHandler,
    filters,
    PollHandler,
    CallbackContext,
    PollAnswerHandler,
)
from telegram.warnings import PTBUserWarning

colorama.init(autoreset=True)

logging.basicConfig(
    level=logging.INFO,
    format='%(levelname)s: %(asctime)s - %(name)s:%(funcName)s - %(message)s',
)

logging.getLogger('httpx').setLevel(logging.WARNING)
filterwarnings(
    action='ignore', message=r'.*CallbackQueryHandler', category=PTBUserWarning
)

tz = timezone(timedelta(hours=3))
dir_main = Path(__file__).parent

RESET = '\033[0m'
BLUE = '\033[34;1m'
CYAN = '\033[36;1m'
GREEN = '\033[32;1m'
MAGENTA = '\033[35;1m'
RED = '\033[31;1m'
YELLOW = '\033[33;1m'

events = {
    'playstation': 'PlayStation',
    'kinect': 'Kinect',
    'vr_bow': 'VR лук',
    'wrestling': 'Треслинг',
    'shooting_gallery': 'Тир',
    'chess': 'Шахматы',
    'tilt_bench': 'Скамья для наклонов',
    'press_bench': 'Скамья для пресса',
    'low_crossbar_for_pushups': 'Низкая перекладина для отжимания',
    'pulling_up_from_the_hanging_position': 'Подтягивание из положения виса',
    'horizontal_bar_for_pulling_up': 'Турник для подтягивания',
    'cancel': 'отмена',
}

keyboard = [
    [
        InlineKeyboardButton(
            callback_data='playstation',
            text='PlayStation',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='kinect',
            text='Kinect',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='vr_bow',
            text='VR лук',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='wrestling',
            text='Треслинг',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='shooting_gallery',
            text='Тир',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='chess',
            text='Шахматы',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='tilt_bench',
            text='Скамья для наклонов',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='press_bench',
            text='Скамья для пресса',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='low_crossbar_for_pushups',
            text='Низкая перекладина для отжимания',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='pulling_up_from_the_hanging_position',
            text='Подтягивание из положения виса',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='horizontal_bar_for_pulling_up',
            text='Турник для подтягивания',
        )
    ],
    [
        InlineKeyboardButton(
            callback_data='cancel',
            text='отмена',
        )
    ],
]

(
    SELECT_EVENT,
    INPUT_ID,
    INPUT_SCORES,
    CONFIRMATION,
    GET_RATING,
) = range(5)

# https://t.me/MrClouds_sport_admin_bot?start=addadmin


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    try:
        param = update.message.text.split(' ')[1]
    except Exception as e:
        logging.error(f'{YELLOW}error:', exc_info=True)
        param = False

    if param == 'addadmin':
        query = (
            'INSERT INTO admins'
            ' (telegram_id, username, first_name, last_name, is_admin)'
            ' VALUES ({}, \'{}\', \'{}\', \'{}\', true)'
            ' ON CONFLICT (telegram_id) DO UPDATE SET is_admin = true;'
        ).format(
            update.effective_user.id,
            update.effective_user.username,
            update.effective_user.first_name,
            update.effective_user.last_name,
        )
        context.user_data['is_admin'] = True
        text = 'Привет. Теперь ты админ. Смотри меню или памятку'
    else:
        query = (
            'INSERT INTO admins'
            ' (telegram_id, username, first_name, last_name)'
            ' VALUES ({}, \'{}\', \'{}\', \'{}\') ON CONFLICT DO NOTHING;'
        ).format(
            update.effective_user.id,
            update.effective_user.username,
            update.effective_user.first_name,
            update.effective_user.last_name,
        )
        context.user_data['is_admin'] = False
        text = 'Драстя'
    try:
        logging.info(f'{CYAN}db query: {query}')
        cursor.execute(query)
    except Exception as e:
        logging.error(f'{RED}db error:', exc_info=True)
        conn.rollback()
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Что-то пошло не так. Попробуй повторить позже',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return
    sent_message = await context.bot.send_message(
        chat_id=update.effective_user.id,
        text=text,
    )
    logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )


async def check_role(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    try:
        query = (
            'SELECT * FROM admins WHERE telegram_id = {} and is_admin = true;'
        ).format(update.effective_user.id)
        logging.info(f'{CYAN}db query: {query}')
        response = cursor.execute(query=query).fetchall()
        logging.info(f'{CYAN}db response: {response}')
    except Exception as e:
        logging.error(f'{RED}db error:', exc_info=True)

        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Что-то пошло не так. Попробуй повторить позже',
        )

        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return

    if response:
        context.user_data['is_admin'] = True
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Привет. Теперь ты админ. Смотри меню или памятку',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
    else:
        context.user_data['is_admin'] = False
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Странно. Но тебя нет в команде админов',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )


async def add_scores(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if context.user_data.get('is_admin') is True:
        pass
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Пройди проверку и возвращайся',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return ConversationHandler.END

    sent_message = await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text='Выбор активности',
        reply_markup=InlineKeyboardMarkup(keyboard),
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )

    return SELECT_EVENT


async def select_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.callback_query.answer()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)
    try:
        await update.callback_query.delete_message()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if update.callback_query.data == 'cancel':
        return ConversationHandler.END

    context.user_data['event'] = update.callback_query.data

    sent_message = await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text='Ввод ID участника',
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )

    return INPUT_ID


async def input_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        int(update.message.text)
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)
        sent_message = await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Неверный ID участника.\nПроверь и отправь ещё раз',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return INPUT_ID

    try:
        query = 'SELECT * FROM participants WHERE participant_id = {};'.format(
            update.message.text
        )
        logging.info(f'{CYAN}db query: {query}')
        response = cursor.execute(query=query).fetchall()
        logging.info(f'{CYAN}db response: {response}')
    except Exception as e:
        logging.error(f'{RED}db error:', exc_info=True)

        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Что-то пошло не так. Попробуй повторить позже',
        )

        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return ConversationHandler.END

    if response:
        context.user_data['participant_id'] = update.message.text
        context.user_data['telegram_id'] = response[0][0]

        sent_message = await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Ввод набранных баллов',
        )

        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )

        return INPUT_SCORES
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Неверный ID участника.\nПроверь и отправь ещё раз',
        )

        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )

        return INPUT_ID


async def input_scores(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        int(update.message.text)
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)
        sent_message = await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Неверные количество баллов.\nПроверь и отправь ещё раз',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return INPUT_SCORES

    context.user_data['scores'] = update.message.text

    sent_message = await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text='Проверяем\n\nАктивность: {}\nID участника: {}\nБаллы: {}'.format(
            events[context.user_data['event']],
            context.user_data['participant_id'],
            context.user_data['scores'],
        ),
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        'верно',
                        callback_data='yes',
                    ),
                    InlineKeyboardButton(
                        'изменить',
                        callback_data='no',
                    ),
                # ],
                # [
                    InlineKeyboardButton(
                        'отмена',
                        callback_data='cancel',
                    ),
                ],
            ]
        ),
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )

    return CONFIRMATION


async def confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.callback_query.answer()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)
    try:
        await update.callback_query.edit_message_reply_markup()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    match update.callback_query.data:
        case 'yes':
            try:
                query = (
                    'INSERT INTO events (telegram_id, {0}) VALUES ({1}, {2})'
                    ' ON CONFLICT (telegram_id) DO UPDATE SET'
                    ' {0} = events.{0} + excluded.{0};'
                ).format(
                    context.user_data['event'],
                    context.user_data['telegram_id'],
                    context.user_data['scores'],
                )
                logging.info(f'{CYAN}db query: {query}')
                cursor.execute(query)
            except Exception as e:
                logging.error(f'{RED}db error:', exc_info=True)
                conn.rollback()
                sent_message = await context.bot.send_message(
                    chat_id=update.effective_user.id,
                    text='Что-то пошло не так. Попробуй повторить позже',
                )
                logging.info(
                    f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
                )
                return ConversationHandler.END
            sent_message = await context.bot.send_message(
                chat_id=update.effective_user.id,
                text='Готово',
            )
            logging.info(
                f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
            )
            return ConversationHandler.END
        case 'no':
            sent_message = await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text='Выбор активности',
                reply_markup=InlineKeyboardMarkup(keyboard),
            )
            logging.info(
                f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
            )
            return SELECT_EVENT
        case _:
            sent_message = await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text='Отмена',
            )
            logging.info(
                f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
            )
            return ConversationHandler.END


async def rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if context.user_data.get('is_admin') is True:
        pass
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Пройди проверку и возвращайся',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return

    sent_message = await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text='Выбор активности',
        reply_markup=InlineKeyboardMarkup(keyboard),
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )

    return GET_RATING


async def get_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.callback_query.answer()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)
    try:
        await update.callback_query.delete_message()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if update.callback_query.data == 'cancel':
        sent_message = await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='отмена',
        )

        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )

        return ConversationHandler.END

    try:
        query = (
            'SELECT e.{0}, p.first_name, p.last_name, p.phone_number,'
            ' p.participant_id, p.username FROM events e JOIN participants p'
            ' ON e.telegram_id = p.telegram_id'
            ' WHERE e.{0} > 0 ORDER BY e.{0} DESC;'
        ).format(update.callback_query.data)
        logging.info(f'{CYAN}db query: {query}')
        response = cursor.execute(query=query).fetchall()
        logging.info(f'{CYAN}db response: {response}')
    except Exception as e:
        logging.error(f'{RED}db error:', exc_info=True)
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Что-то пошло не так. Попробуй повторить позже',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return ConversationHandler.END
    text = '{}\n'.format(events[update.callback_query.data])
    text += 'место. рейтинг / имя фамилия / телефон / id / telegram\n\n'
    if len(response) == 0:
        counter = 0
    elif len(response) < 10:
        counter = len(response)
    else:
        counter = 10

    if counter:
        for i in range(counter):
            if response[i][5] == 'None':
                text += '{}. {} / {} {} / {} / {}\n'.format(
                    i + 1,
                    response[i][0],
                    response[i][1],
                    response[i][2],
                    response[i][3],
                    response[i][4],
                )
            else:
                text += '{}. {} / {} {} / {} / {} / @{}\n'.format(
                    i + 1,
                    response[i][0],
                    response[i][1],
                    response[i][2],
                    response[i][3],
                    response[i][4],
                    response[i][5],
                )
        # try:
        #     query = (
        #         'SELECT {0} FROM events WHERE telegram_id = {1} and {0} > 0;'
        #     ).format(update.callback_query.data, update.effective_user.id)
        #     logging.info(f'{CYAN}db query: {query}')
        #     response = cursor.execute(query=query).fetchall()
        #     logging.info(f'{CYAN}db response: {response}')
        # except Exception as e:
        #     logging.error(f'{RED}db error:', exc_info=True)

        #     sent_message = await context.bot.send_message(
        #         chat_id=update.effective_user.id,
        #         text='Что-то пошло не так. Попробуй повторить позже',
        #     )

        #     logging.info(
        #         f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        #     )
        #     return ConversationHandler.END
        # if response:
        #     text += '\nТвой рейтинг: {}'.format(response[0][0])
        # else:
        #     text += '\nТы ещё не участвовал'
    else:
        text += 'Ещё никто не участвовал'

    sent_message = await context.bot.send_message(
        chat_id=update.effective_user.id,
        text=text,
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )

    return ConversationHandler.END


async def get_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if context.user_data.get('is_admin') is True:
        pass
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Пройди проверку и возвращайся',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return

    text = (
        'Топ-1 по всем активностям\n'
        'рейтинг / имя фамилия / телефон / id / telegram\n\n'
    )
    for event in events.keys():
        if event == 'cancel':
            pass
        else:
            try:
                query = (
                    'SELECT e.{0}, p.first_name, p.last_name, p.phone_number,'
                    ' p.participant_id, p.username FROM events e'
                    ' JOIN participants p ON e.telegram_id = p.telegram_id'
                    ' WHERE e.{0} = (select max({0}) from events);'
                ).format(event)
                logging.info(f'{CYAN}db query: {query}')
                response = cursor.execute(query=query).fetchall()
                logging.info(f'{CYAN}db response: {response}')
            except Exception as e:
                logging.error(f'{RED}db error:', exc_info=True)

                sent_message = await context.bot.send_message(
                    chat_id=update.effective_user.id,
                    text='Что-то пошло не так. Попробуй повторить позже',
                )

                logging.info(
                    f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
                )
                return
            if response:
                if response[0][0] == 0:
                    text += '{}:\nнет участников\n\n'.format(
                        events[event],
                    )
                elif response[0][5] == 'None':
                    text += '{}:\n{} / {} {} / {} / {}\n\n'.format(
                        events[event],
                        response[0][0],
                        response[0][1],
                        response[0][2],
                        response[0][3],
                        response[0][4],
                    )
                else:
                    text += '{}:\n{} / {} {} / {} / {} / @{}\n\n'.format(
                        events[event],
                        response[0][0],
                        response[0][1],
                        response[0][2],
                        response[0][3],
                        response[0][4],
                        response[0][5],
                    )

    sent_message = await context.bot.send_message(
        chat_id=update.effective_user.id,
        text=text,
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )


async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    sent_message = await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text='Отмена',
    )

    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )
    # context.user_data.clear()

    return ConversationHandler.END


async def get_checkin_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if context.user_data.get('is_admin') is True:
        pass
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Пройди проверку и возвращайся',
        )
        return

    try:
        query = 'SELECT count(*) from checkin;'
        logging.info(f'{CYAN}db query: {query}')
        response = cursor.execute(query=query).fetchall()
        logging.info(f'{CYAN}db response: {response}')
    except Exception as e:
        logging.error(f'{RED}db error:', exc_info=True)
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Что-то пошло не так. Попробуй повторить позже',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return

    sent_message = await context.bot.send_message(
        chat_id=update.effective_user.id,
        text='Всего зачекинилось: {}'.format(response[0][0]),
    )
    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )


async def get_checkin_report(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if context.user_data.get('is_admin') is True:
        pass
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Пройди проверку и возвращайся',
        )
        return

    try:
        query = (
            'SELECT p.participant_id, p.first_name, p.last_name, p.phone_number,'
            ' p.username FROM checkin c JOIN participants p'
            ' ON c.telegram_id = p.telegram_id ORDER BY p.last_name asc;'
        )
        logging.info(f'{CYAN}db query: {query}')
        response = cursor.execute(query=query).fetchall()
        logging.info(f'{CYAN}db response: {response}')
    except Exception as e:
        logging.error(f'{RED}db error:', exc_info=True)
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Что-то пошло не так. Попробуй повторить позже',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return

    participants = []
    for line in response:
        participants.append(
            [
                line[0],
                line[1],
                line[2],
                line[3],
                '' if line[4] == 'None' else f'@{line[4]}',
            ]
        )

    report = pandas.DataFrame(
        data=participants,
        columns=[
            'id',
            'имя',
            'фамилия',
            'телефон',
            'telegram',
        ],
    )

    file_name = 'checkin_report_{}.xlsx'.format(
        datetime.strftime(datetime.now(tz=tz), "%Y%m%d%H%M")
    )
    dir_main = Path(__file__).parent
    with pandas.ExcelWriter(path=Path(dir_main, 'data', file_name)) as writer:
        report.to_excel(writer, sheet_name='отчёт', index=False)

    sent_message = await context.bot.send_document(
        chat_id=update.effective_user.id,
        document=Path(dir_main, 'data', file_name),
    )
    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )


async def get_events_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    if context.user_data.get('is_admin') is True:
        pass
    else:
        sent_message = await context.bot.send_message(
            chat_id=update.effective_user.id,
            text='Пройди проверку и возвращайся',
        )
        logging.info(
            f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
        )
        return

    participants = []
    for event in events.keys():
        if event == 'cancel':
            pass
        else:
            try:
                query = (
                    'SELECT e.{0}, p.first_name, p.last_name, p.phone_number,'
                    ' p.participant_id, p.username FROM events e JOIN participants p'
                    ' ON e.telegram_id = p.telegram_id'
                    ' WHERE e.{0} > 0 ORDER BY e.{0} DESC;'
                ).format(event)
                logging.info(f'{CYAN}db query: {query}')
                response = cursor.execute(query=query).fetchall()
                logging.info(f'{CYAN}db response: {response}')
            except Exception as e:
                logging.error(f'{RED}db error:', exc_info=True)

                sent_message = await context.bot.send_message(
                    chat_id=update.effective_user.id,
                    text='Что-то пошло не так. Попробуй повторить позже',
                )

                logging.info(
                    f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
                )
                return
            participants.append([events[event]])
            for line in response:
                if line[0] == 0:
                    pass
                else:
                    participants.append(
                        [
                            line[0],
                            line[1],
                            line[2],
                            line[3],
                            line[4],
                            '' if line[5] == 'None' else f'@{line[5]}',
                        ]
                    )
            participants.append([])

    report = pandas.DataFrame(
        data=participants,
        columns=[
            'баллы',
            'id',
            'имя',
            'фамилия',
            'телефон',
            'telegram',
        ],
    )

    file_name = 'events_report_{}.xlsx'.format(
        datetime.strftime(datetime.now(tz=tz), "%Y%m%d%H%M")
    )
    dir_main = Path(__file__).parent
    with pandas.ExcelWriter(path=Path(dir_main, 'data', file_name)) as writer:
        report.to_excel(writer, sheet_name='отчёт', index=False)

    sent_message = await context.bot.send_document(
        chat_id=update.effective_user.id,
        document=Path(dir_main, 'data', file_name),
    )
    logging.info(
        f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    )


async def get_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.message.delete()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)

    # sent_message = await context.bot.send_poll(
    #     chat_id=update.effective_user.id,
    #     question='wtf',
    #     options=['f', 'kk', 'ggwp'],
    #     is_anonymous=False,
    #     allows_multiple_answers=False,
    # )
    # logging.info(
    #     f'{MAGENTA}sent message to {YELLOW}{update.effective_user.id}:\n{GREEN}{sent_message}'
    # )


# async def poll_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     logging.info(
#         f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
#     )


async def msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )


async def handle_invalid_button(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    logging.info(
        f'{MAGENTA}received message from {YELLOW}{update.effective_user.id}:\n{GREEN}{update}'
    )

    try:
        await update.callback_query.answer()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)
    try:
        await update.callback_query.delete_message()
    except Exception as e:
        logging.error(f'{RED}error:', exc_info=True)


def main():
    app.add_handler(
        ConversationHandler(
            entry_points=[
                CommandHandler(
                    command='addscores',
                    callback=add_scores,
                    filters=filters.ChatType.PRIVATE,
                ),
            ],
            states={
                SELECT_EVENT: [
                    CallbackQueryHandler(
                        callback=select_event,
                        # pattern='^event='
                    ),
                ],
                INPUT_ID: [
                    MessageHandler(
                        filters=filters.TEXT
                        & ~filters.COMMAND
                        & filters.ChatType.PRIVATE,
                        callback=input_id,
                    ),
                ],
                INPUT_SCORES: [
                    MessageHandler(
                        filters=filters.TEXT
                        & ~filters.COMMAND
                        & filters.ChatType.PRIVATE,
                        callback=input_scores,
                    ),
                ],
                CONFIRMATION: [
                    CallbackQueryHandler(
                        callback=confirmation,
                        # pattern='^no$'
                    ),
                ],
            },
            fallbacks=[
                CommandHandler(
                    command='cancel',
                    callback=cancel,
                    filters=filters.ChatType.PRIVATE,
                ),
            ],
        )
    )
    app.add_handler(
        ConversationHandler(
            entry_points=[
                CommandHandler(
                    command='rating',
                    callback=rating,
                    filters=filters.ChatType.PRIVATE,
                ),
            ],
            states={
                GET_RATING: [
                    CallbackQueryHandler(
                        callback=get_rating,
                    ),
                ],
            },
            fallbacks=[
                CommandHandler(
                    command='cancel',
                    callback=cancel,
                    filters=filters.ChatType.PRIVATE,
                ),
            ],
        )
    )
    app.add_handler(
        CommandHandler(
            command='start',
            callback=start,
            filters=filters.ChatType.PRIVATE,
        )
    )
    app.add_handler(
        CommandHandler(
            command='admin',
            callback=check_role,
            filters=filters.ChatType.PRIVATE,
        )
    )
    app.add_handler(
        CommandHandler(
            command='checkin',
            callback=get_checkin_count,
            filters=filters.ChatType.PRIVATE,
        )
    )
    app.add_handler(
        CommandHandler(
            command='help',
            callback=get_help,
            filters=filters.ChatType.PRIVATE,
        )
    )
    app.add_handler(
        CommandHandler(
            command='top',
            callback=get_top,
            filters=filters.ChatType.PRIVATE,
        )
    )
    app.add_handler(
        CommandHandler(
            command='checkin_report',
            callback=get_checkin_report,
            filters=filters.ChatType.PRIVATE,
        )
    )
    app.add_handler(
        CommandHandler(
            command='events_report',
            callback=get_events_report,
            filters=filters.ChatType.PRIVATE,
        )
    )
    # app.add_handler(PollHandler(callback=poll_handler))
    # app.add_handler(PollAnswerHandler(callback=poll_handler))
    app.add_handler(
        MessageHandler(
            filters=filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE,
            callback=msg,
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            handle_invalid_button,
        )
    )

    app.run_polling(allowed_updates=Update.ALL_TYPES)


if __name__ == '__main__':
    bot_name = (
        os.getenv('BOT_NAME')
        if os.getenv('BOT_NAME')
        else 'MrClouds sport admin TEST bot'
    )
    token = (
        os.getenv('TOKEN')
        if os.getenv('TOKEN')
        else '7216920370:AAFsaROLFUjWOmViJdyf3svrRrL2ThW-2-4'
    )
    db_name = (
        os.getenv('DB_NAME')
        if os.getenv('DB_NAME')
        else 'rtfest_summer_2024_test'
    )
    db_host = os.getenv('DB_HOST') if os.getenv('DB_HOST') else '192.168.1.250'

    # tz = timezone(timedelta(hours=3))

    logging.info(
        f'{YELLOW}\n    bot name: {bot_name};\n    db name: {db_name};\n    db host: {db_host}'
    )

    app = Application.builder().token(token).build()

    conn = psycopg.connect(
        autocommit=True,
        dbname=db_name,
        user='postgres',
        password='1I1DG5reb8Cf9BeP',
        host=db_host,
        port=5432,
    )

    cursor = conn.cursor()

    main()
